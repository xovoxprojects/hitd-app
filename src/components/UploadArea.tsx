import { UploadCloud, CheckCircle2, AlertCircle, FileImage, FileVideo, Sparkles, UserCircle2 } from "lucide-react";
import { useState, useRef } from "react";
import { supabase } from "@/lib/supabase";

type Mode = "visual" | "copy" | "bio";
...
export default function UploadArea({ onAnalysisComplete }: { onAnalysisComplete: (data: any) => void }) {
  ...
  // For bio mode we prefix the text so the AI knows the context
  const buildText = () => {
    if (mode === "bio") {
      const ctx = bioContext.trim() ? `\n\nContexto adicional: ${bioContext.trim()}` : "";
      return `[ANÁLISIS DE BIOGRAFÍA DE PERFIL]\n\n${textContent}${ctx}`;
    }
    return textContent;
  };

  const analyze = async () => {
    if (mode !== "visual" && !textContent.trim()) {
      setError("Por favor, escribí o pegá el texto para analizar.");
      return;
    }
    if (mode === "visual" && !file && !textContent.trim()) {
  ...
  const isDisabled = analyzing || (
    mode === "visual" ? (!file && !textContent.trim()) : !textContent.trim()
  );
  ...
  return (
    <div className="bg-white/80 backdrop-blur-xl border border-slate-200/80 rounded-[2rem] p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden group">
      ...
      {/* Header + tabs */}
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-8 relative z-10 gap-4">
        <h2 className="text-2xl font-bold text-slate-900 border-l-4 border-indigo-600 pl-4 tracking-tight">Nuevo Análisis</h2>
        <div className="bg-slate-100 rounded-xl p-1.5 flex shadow-inner border border-slate-200/60 self-start sm:self-auto">
          <button
            onClick={() => { setMode("visual"); setTextContent(""); setFile(null); }}
            className={`px-5 py-2 text-sm rounded-lg transition-all duration-300 ${mode === "visual" ? 'bg-white text-indigo-700 font-bold shadow-sm' : 'text-slate-500 hover:text-slate-900 font-semibold'}`}
          >
            Creativo Visual
          </button>
          <button
            onClick={() => { setMode("copy"); setFile(null); }}
            className={`px-5 py-2 text-sm rounded-lg transition-all duration-300 ${mode === "copy" ? 'bg-white text-indigo-700 font-bold shadow-sm' : 'text-slate-500 hover:text-slate-900 font-semibold'}`}
          >
            Solo Copy
          </button>
          <button
            onClick={() => { setMode("bio"); setFile(null); }}
            className={`px-5 py-2 text-sm rounded-lg transition-all duration-300 flex items-center gap-1.5 ${mode === "bio" ? 'bg-white text-indigo-700 font-bold shadow-sm' : 'text-slate-500 hover:text-slate-900 font-semibold'}`}
          >
      ...
      <div className="relative z-10">
        ...
        {mode === "visual" && (
          <div className="flex flex-col gap-4">
            ...
            <textarea
              ...
              placeholder="Añadí contexto opcional o copy del anuncio (ej: texto del anuncio, público objetivo, etc.)"
        ...
        {mode === "copy" && (
          <div className="relative group/inner">
            <div className="absolute -inset-1 bg-gradient-to-r from-blue-300 to-indigo-300 rounded-[2.2rem] blur opacity-0 group-focus-within:opacity-30 transition duration-500" />
            <textarea
              value={textContent}
              onChange={(e) => setTextContent(e.target.value)}
        ...
        {mode === "bio" && (
          <div className="flex flex-col gap-4">
            {/* Info banner */}
            <div className="flex items-start gap-3 px-5 py-4 bg-indigo-50 border border-indigo-100 rounded-2xl text-sm text-indigo-700 font-medium">
              <UserCircle2 className="w-5 h-5 shrink-0 mt-0.5 text-indigo-500" />
              <span>
                Pegá la <strong>biografía</strong> de tu perfil o página de Instagram/Facebook para detectar frases que puedan violar las políticas de Meta y reducir el alcance orgánico o publicitario de tu cuenta.
              </span>
            </div>

            <div className="relative group/inner">
              <div className="absolute -inset-1 bg-gradient-to-r from-indigo-300 to-purple-300 rounded-[2.2rem] blur opacity-0 group-focus-within:opacity-30 transition duration-500" />
              <textarea
                value={textContent}
                onChange={(e) => setTextContent(e.target.value)}
            ...
            <textarea
              ...
              placeholder="Contexto adicional (opcional): nicho, tipo de cuenta (personal, negocio, creador), objetivo..."
              className="w-full h-20 bg-slate-50 border border-slate-200 rounded-[1.5rem] p-5 text-slate-800 text-sm font-medium placeholder-slate-400 focus:outline-none focus:border-indigo-400 focus:bg-white resize-none transition-colors"
            />
          </div>
        )}
      </div>
      ...
      <div className="mt-10 flex flex-col sm:flex-row items-center justify-between border-t border-slate-100 pt-8 relative z-10">
        ...
        >
          {analyzing ? (
            ...
            <>
              <div className="absolute inset-0 h-full w-full bg-gradient-to-r from-blue-600 to-indigo-600 opacity-0 group-hover/btn:opacity-100 transition-opacity duration-300" />
              <span className="relative z-10 flex items-center justify-center gap-2">
                {mode === "bio" ? "Analizar Biografía" : "Ejecutar Análisis"} <Sparkles className="w-4 h-4" />
              </span>
            </>
          )}
        </button>
